...
func TestGetDummyUserByID1(t *testing.T) {
    ...
    case 200:
        ...
        // Verify the user has ID 1
        assert.Equal(t, 1, fetchedUser.ID)
        assert.NotEmpty(t, fetchedUser.Name, "User name should not be empty")
        assert.NotEmpty(t, fetchedUser.Email, "User email should not be empty")
        
        t.Logf("Found user ID 1: Name='%s', Email='%s'", fetchedUser.Name, fetchedUser.Email)
    case 500:
        // User with ID 1 does not exist (JSON service returns 500 for "user not found")
        t.Log("User with ID 1 not found - this is expected if no initial data exists")
    default:
