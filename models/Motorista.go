...
type Motorista struct {
    gorm.Model                                                      // Mantido do Motorista.go
    ID             string            `json:"id"`                      // Alterado para uint (GORM padrão)
    Nome           string          `json:"nome" validate:"required,min=2,max=100"`
    DataNascimento time.Time       `json:"data_nascimento" validate:"required"`
    CPF            string          `json:"cpf" validate:"required"` // Mantido como string (mais flexível)
    Idade          *int            `json:"idade"`                   // Incorporado do Motorista.go (opcional, calculado)
    ...
    CRLV           *string         `json:"crlv"`                    // Incorporado do Motorista.go (opcional)
...
type Documento struct {
    gorm.Model                                                   // Adicionado para consistência GORM
    ID             string      `json:"id"`                       // Alterado para uint
    MotoristaID    uint      `json:"motorista_id"`               // FK para Motorista
